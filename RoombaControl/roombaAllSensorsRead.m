function [BumpRight, BumpLeft, BumpFront, Wall, virtWall, CliffLft, ...
    ...
    Volts, Current, Temp, Charge, Capacity, pCharge]   = roombaAllSensorsRead()
...
% Get all data fields [142 0]
roombaSendMessage([sprintf('\x8E') 0])

% Read data fields
% BmpWheDrps = dec2bin(fread(serPort, 1),8);  %
BmpWheDrps = dec2bin(roombaReceiveMessage(1), 8);  %
...
Wall = roombaReceiveMessage(1)  %0 no wall, 1 wall

CliffLft = roombaReceiveMessage(1) % no cliff, 1 cliff
CliffFrntLft = roombaReceiveMessage(1)
CliffFrntRgt = roombaReceiveMessage(1)
CliffRgt = roombaReceiveMessage(1)

virtWall = roombaReceiveMessage(1)%0 no wall, 1 wall

motorCurr = dec2bin( roombaReceiveMessage(1) ,8 );
...
DirtL = roombaReceiveMessage(1)
DirtR = roombaReceiveMessage(1)

RemoteCode =  roombaReceiveMessage(1); % coudl be used by remote or to communicate with sendIR command
Buttons = dec2bin(roombaReceiveMessage(1) ,8);
...
Dist = roombaReceiveMessage(1)/1000 % convert to Meters, signed, average dist wheels traveled since last time called...caps at +/-32

% Angle = fread(serPort, 1, 'int16')*pi/180 % convert to radians, signed,  since last time called, CCW positive
Angle = roombaReceiveMessage(1)*pi/180 % convert to radians, signed,  since last time called, CCW positive

ChargeState = roombaReceiveMessage(1)
% Volts = fread(serPort, 1, 'uint16')/1000
Volts = roombaReceiveMessage(1)/1000

% Current = fread(serPort, 1, 'int16')/1000 % neg sourcing, pos charging
Current = roombaReceiveMessage(1)/1000 % neg sourcing, pos charging

% Temp  =  fread(serPort, 1, 'int8') 
Temp  =  roombaReceiveMessage(1)

% Charge =  fread(serPort, 1, 'uint16') % in mAhours
Charge =  roombaReceiveMessage(1) % in mAhours
...
checksum = roombaReceiveMessage(1)
