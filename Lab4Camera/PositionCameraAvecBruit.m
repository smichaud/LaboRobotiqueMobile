...
% Parametre de bruit sur la position en pixel
spixel = 2;
...
% Simulation de bruit sur la mesure des pixels
for iSample = 1:1000
    % Calcul des angles
    NoisyLx = Lx + spixel*randn(3,1);
    theta = atan((NoisyLx-u0)/f);
    ...
    [Pose1 Pose2] = circleintersect(c1',r1,c2',r2,'lr');
    % On conserve celui le plus loin de 0,0. Hack rapide pour eliminer
    % la solution proche des reperes.
    if (norm(Pose2)>norm(Pose1))
        PoseRobot = Pose2;
    else
        PoseRobot = Pose1;
    end
    
    plot(PoseRobot(1),PoseRobot(2),'.','MarkerSize',1);
    hold on;
end
...
plot(PoseRobot(1),PoseRobot(2),'ko','MarkerFace','w');
